<script lang="ts">
  ...
  function draw() {
    ...
    stars.forEach((star) => {
      ...
      // Modulo to wrap stars around screen for infinite background
      let parallaxX = (star.x - camera.x * 0.05) % canvas.width;
      let parallaxY = (star.y - camera.y * 0.05) % canvas.height;
