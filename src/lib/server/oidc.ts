...
async function exchangeCodeForTokens(code: string, redirectUri: string): Promise<OidcToken | null> {
  try {
    ...
    if (!response.ok) {
      console.error('[OIDC] Code exchange failed:', response.status, await response.text());
      return null;
    }
...
async function fetchUserProfile(accessToken: string): Promise<OidcProfile | null> {
  try {
    ...
    if (!response.ok) {
      console.error('[OIDC] userinfo fetch failed:', response.status, await response.text());
      return null;
    }
...
export async function completeOIDCFlow(
  ...
  let firstName = trimmedOrNull(profile.firstName) ?? trimmedOrNull(profile.given_name);
  let lastName = trimmedOrNull(profile.lastName) ?? trimmedOrNull(profile.family_name);
  ...
  const formation = trimmedOrNull(profile.formation) ?? trimmedOrNull(idClaims.formation);
