...
describe('the search contract', () => {
  ...
  describe('the tolerance ladder', () => {
    ...
    it('allows a second edit from eight characters', () => {
      expect(personMatchScore('Dupontier', 'Jean', 2024, 'dupontlar')).not.toBeNull();
    });
  });
  ...
  describe('every typed word must match something', () => {
    it('rejects a person who answers only half the query', () => {
      expect(personMatchScore('Dupont', 'Jean', 2024, 'jean dupont')).not.toBeNull();
      expect(personMatchScore('Dupont', 'Jean', 2024, 'jean martin')).toBeNull();
    });

    it('does not care about word order', () => {
      expect(personMatchScore('Dupont', 'Jean', 2024, 'dupont jean')).not.toBeNull();
    });
  });
