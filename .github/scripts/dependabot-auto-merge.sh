...
while IFS='|' read -r name type version; do
  ...
  # `@sveltejs/*`, `@humanspeak/*`, `@lucide/*`, `@tailwindcss/*`, `@inlang/*`. A ceiling entry
  # written without this stripping would match none of them and refuse nothing.
  name="${name%\"}"
  name="${name#\"}"
  gate=""
  case "$name" in
    # THIS LIST IS EMPTY, AND THAT IS A MEASURED ANSWER RATHER THAN AN OMISSION.
    #
    ...
    # check`, `bun run lint` and `bun run build`, and merges on its own. Three candidates were
    # measured here on 2026-08-31, and all three were closed by writing the test rather than by
    # refusing the update - which is what a refusal is for.
    #
    #   graphology, graphology-components, graphology-layout, graphology-layout-forceatlas2
    #     A layout library does not stop compiling when it changes, it RETURNS DIFFERENT NUMBERS.
    #     Nothing tested `layoutGraph` at all, so the star map could have silently rearranged
    #     itself for every student behind a green pipeline. `src/lib/server/positions.test.ts`
    #     freezes the map a fixed graph produces.
    #
    #   @sveltejs/adapter-node
    #     A build is not a boot. `bun run build` proved the adapter produced something; nothing ran
    #     the thing it produced. The `Boot the packaged server` step in `ci-bun.yml` starts
    #     `build/index.js` and asks it for `/api/health`.
    #
    #   @humanspeak/svelte-markdown
    #     It renders text this application did not write. The component ASSERTED in a comment that
    #     raw HTML was escaped; measured, it was not - `<iframe src=...>` came through intact.
    #     `src/lib/components/BioMarkdown.test.ts` now holds that property down.
    #
    # `tar` deliberately has no entry: it is used only by `scripts/pack-bun.js`, which runs in the
    # `Package application` step on main, so a break there reddens CI and blocks the deploy. Late,
    # but not invisible - and invisibility is the criterion.
    #
