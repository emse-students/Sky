...
if (fs.existsSync(dataDir)) {
  ...
  console.warn('⚠️  database/ not found - the package will not include a database');
}

const envFile = path.resolve(process.cwd(), '.env');
const includeEnv = String(process.env.PACK_INCLUDE_ENV || '').toLowerCase() === 'true';
if (fs.existsSync(envFile) && includeEnv) {
  console.log('✅ .env found and inclusion requested via PACK_INCLUDE_ENV=true');
  ...
  console.warn('⚠️  .env not found - the package will not include configuration');
}
