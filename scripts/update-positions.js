...
const newPositionsPath = path.join(__dirname, '../static/data/positions.json.new');
...
console.log(`Anciennes positions : ${Object.keys(oldPositions).length} entrées`);
...
const mapping = {};
const allPeople = db.prepare('SELECT id, first_name, last_name FROM people').all();
...
Object.entries(mapping).slice(0, 5).forEach(([old, newId]) => {
...
fs.writeFileSync(oldPositionsPath, JSON.stringify(newPositions, null, 2), 'utf-8');
