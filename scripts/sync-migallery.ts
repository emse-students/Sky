 ...
 */
async function fetchFromMiGallery(personId: string): Promise<MiGalleryPerson | null> {
	// TODO: Replace with actual MiGallery API endpoint
	const MIGALLERY_API = process.env.MIGALLERY_API_URL || 'https://api.migallery.example/';
 ...
 */
async function downloadProfilePicture(photoUrl: string, personId: string): Promise<string> {
...
export async function syncAllProfilePictures(): Promise<void> {
	...
	for (const person of people) {
		try {
			...
			const imageFilename = await downloadProfilePicture(miGalleryData.photo_url, person.id);
 ...
 */
export async function syncPersonProfilePicture(personId: string): Promise<boolean> {
	...
	try {
		...
		const imageFilename = await downloadProfilePicture(miGalleryData.photo_url, personId);
