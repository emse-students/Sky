...
describe("absolute URLs", () => {
  it("builds the preview image and the canonical from the request origin", () => {
    expect(defaultImage("https://sky.mitv.fr")).toBe(
      "https://sky.mitv.fr/sky.png",
    );
    expect(canonicalUrl("http://localhost:5173", "/")).toBe(
      "http://localhost:5173/",
    );
  });
...
describe("serializeJsonLd", () => {
  ...
  it("cannot close the script element it is embedded in", () => {
    ...
    expect(JSON.parse(hostile)["@graph"][0].name).toBe(
...
describe("prune", () => {
  it("removes undefined, null and empty members and keeps falsy-but-real ones", () => {
    expect(
      prune({ a: "x", b: null, c: undefined, d: "", e: 0, f: false }),
    ).toEqual({
      a: "x",
      e: 0,
      f: false,
